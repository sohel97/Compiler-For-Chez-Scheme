...
let make_prologue consts_tbl fvars_tbl =
  let get_const_address const = let (a,(b,c)) = (List.find (fun (a,(b,c)) -> (expr_eq (add_const a) (add_const const) )) consts_tbl) in
                                "const_tbl+" ^ (string_of_int b) in
  let get_fvar_address const =  let (a,b) = (List.find (fun (a,b) -> a = const) fvars_tbl) in
                                  (string_of_int b) in
